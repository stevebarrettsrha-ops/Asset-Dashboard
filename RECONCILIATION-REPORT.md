...
**Hotfix (2026-09-03, same day):** on a phone the first run of the pass froze the login screen. Two causes: it ran synchronously inside the boot sequence, before the login form could take input, and it wrote one audit entry per created asset — each of which re-saved the whole audit trail — plus a tombstone/ledger write per row. Measured at 4× CPU throttle with a 12,000-entry audit trail, the page was blocked for 33 s. Now the pass is deferred until the page is idle (login is usable within ~2 s at the same throttle), all tombstone, ledger and audit writes are batched into one each, the view re-render runs in its own task, and a circuit breaker pauses automatic runs for the session if the pass keeps finding changes (a sign that something else is undoing them) so a device can never be caught in a refresh loop. Reconcile Locations on the Asset Register still runs the pass on demand.

**Correction (2026-09-03, later the same day):** the first version of the pass created a register asset for every coded sheet row with no register match. On a live device that was about 2,600 rows — assets that had been deleted from the register on purpose while their survey rows stayed on the sheets — all dated the day the pass ran and with no category or source, so the dashboard's fiscal-year counts jumped by thousands (2,704 assets, 2,621 "medical", 2,635 "donated"). The pass no longer creates or deletes anything on its own: a row with no register asset is counted and left alone, and the Reconcile Locations report offers **Add to register** and **Remove from Location Records** for those rows so a person decides. Every asset the first version minted is recognisable (deterministic `loc_` id, survey origin, no cost, none of the marks a person's own action leaves) and is removed by the pass wherever it turns up, including when it later arrives from a cloud backup, with its id tombstoned so no merge brings it back; the sheet rows that pointed at it stay and are unlinked. Assets added deliberately from the report or by coding a row on the Location Records page are marked `surveyAdded` and are kept.
